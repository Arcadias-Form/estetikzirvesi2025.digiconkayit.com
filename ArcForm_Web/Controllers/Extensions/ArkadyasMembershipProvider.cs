...
namespace VeritabaniIslemMerkezi
{
    public class ArkadyasMembershipProvider : MembershipProvider
    {
        ...
        {
            VTOperatorleri VTIslem = new VTOperatorleri("SELECT COUNT(*) FROM KullaniciTablosu WHERE ePosta=@ePosta AND Sifre=@Sifre");
    ...
    public class ArkadyasRoleProvider : RoleProvider
    {
        ...
        {
            VTOperatorleri VTIslem = new VTOperatorleri("SELECT COUNT(*) FROM KullaniciTablosu WHERE ePosta=@ePosta");
            VTIslem.AddWithValue("ePosta", username);
            return Convert.ToInt32(VTIslem.ExecuteScalar()).Equals(1) ? new string[] { "Admin" } : null;
        }
