            ...
            using (MimeMessage mm = new MimeMessage())
            {
               ...
               mm.Bcc.Add(new MailboxAddress("Estetik Zirvesi 2025", "estetikzirvesi@rubikonturizm.com"));
                ...
                switch (DilID)
                {
                    default:
                    case "tr":
                        mm.Subject = "Estetik Zirvesi 2025 Kayýt Bilgilendirme";

                        OutlookContent = new StringBuilder()
                            .Append($"<p>Sayýn {YeniKayit.KatilimciBilgisi.AdSoyad},</p>")
                            .Append($"<div><p>A' dan Z'ye Estetik Zirvesine kayýt yaptýrdýðýnýz için teþekkür ederiz.</p>{(YeniKayit.OdemeTipiID.Equals(1) ? $"<p>Ön kaydýnýz, ödeme sonrasý dekontunuzu ilettiðiniz de onaylanacaktýr.</p><p><u><b>Ödeme tutarý, ödeme yaptýðýnýz tarihteki dönem ücretine göre yapýlmalýdýr.</b></u> Aksi taktirde ön kaydýnýz onaylanmayacaktýr.</p><p>Ödemeyi, <u><b>sipariþ numarasý</b></u>'ný dekontunuzda belirterek yapmayý unutmayýnýz. Dekontunuzu <a href=\"mailto:estetikzirvesi@rubikonturizm.com\">estetikzirvesi@rubikonturizm.com</a> adresine iletmeyi unutmayýnýz.</p><p></p>" : string.Empty)}<p>Katýlým bilgileriniz aþaðýdadýr:</p></div>")
                        ...
                        if (YeniKayit.OdemeTipiID.Equals(1))
                        {
                            OutlookContent
                                .Append("<tr><td colspan=\"2\">&nbsp;</td></tr>")
                                .Append($"<tr><td colspan=\"2\" style=\"{textCenter}\"><b>Banka Havale Bilgileri</b></td></tr>")
                                .Append($"<tr><td>Hesap Adý</td><td>Rubikon Turizm Organizasyon Hizmetleri Tic. Ltd. Þti </td></tr>")
                                .Append($"<tr><td>Banka</td><td> Yapý Kredi Bankasý </td></tr>")
                                .Append($"<tr><td>Þube</td><td> Esentepe Büyükdere Caddesi Þubesi </td></tr>")
                                .Append($"<tr><td>Hesap No [ TL ]</td><td> TR38 0006 7010 0000 0095 1953 11</td></tr>")
                                .Append($"<tr><td>Hesap No[ EURO ] </td><td> TR26 0006 7010 0000 0095 1978 55 </td></tr>")
                                .Append($"<tr><td>Banka</td><td> Rubikon Turizm / ESTETÝK ZÝRVESÝ 2025 </td></tr>");
                        }
                    ...
                    case "en":
                        ...
                        OutlookContent = new StringBuilder()
                            ...
                            .Append($"<tr><td>Registration Type</td><td>{YeniKayit.KatilimciBilgisi.KatilimciTipiBilgisi.KatilimciTipiDilBilgisi.First().KatilimciTipi}</td></tr>")
                            .Append($"<tr><td>Accommoadtion</td><td>{YeniKayit.KatilimciBilgisi.KonaklamaBilgisi.OdaTipiBilgisi.OtelBilgisi.Otel} / {YeniKayit.KatilimciBilgisi.KonaklamaBilgisi.OdaTipiBilgisi.OdaTipiDilBilgisi.First().OdaTipi}</td></tr>")
                ...
                using (ProtocolLogger logger = new ProtocolLogger(HttpContext.Current.Server.MapPath($"~/Dosyalar/MailLog/{YeniKayit.KatilimciBilgisi.ePosta}_{DateTime.Now:yyyy.MM.dd HH.mm.ss}.log")))
                {
                    using (SmtpClient client = new SmtpClient(logger))
                    {
                        try
                        {
                            ...
                            client.Authenticate("estetikzirvesi2025@digiconkayit.com", "!kfpuc8W");
