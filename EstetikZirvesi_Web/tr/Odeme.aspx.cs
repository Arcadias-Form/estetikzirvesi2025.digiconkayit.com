...
namespace EstetikZirvesi_Web.tr
{
    public partial class Odeme : Page
    {
        ...
        {
            Kontrol.BoolKontrol(ddlKrediKartiUlke.SelectedValue, "Kredi kartýnýzýn ait olduðu ülkeyi seçiniz", "Geçersiz ülke seçildi", ref Uyarilar);
            Kontrol.KelimeKontrol(txtKrediKartNo, "Kredi kart numarasý boþ býrakýlamaz", ref Uyarilar);
            Kontrol.KelimeKontrol(txtAy, "Ay boþ býrakýlamaz", ref Uyarilar);
            Kontrol.KelimeKontrol(txtYil, "Yýl boþ býrakýlamaz", ref Uyarilar);
            Kontrol.KelimeKontrol(txtCVV2, "CVV2 boþ býrakýlamaz", ref Uyarilar);

            if (string.IsNullOrEmpty(Uyarilar.ToString()))
            {
                // Bankaya göre kod düzeneði gelecek.
            }
            else
            {
